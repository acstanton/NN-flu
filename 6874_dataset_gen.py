...
# Allie Stanton
# 
# In this module I will read in the fasta files of HA sequences and convert them to one-hot encoding readable by TensorFlow.
# 
...
############################################################################
    ...
    for i in range(1, num_subtypes + 1):
        ...
        for record in SeqIO.parse(file, 'fasta'):
            seq = str(record.seq)
            seq_onehot = sequence_to_one_hot(seq)
...
padded, labels = pad_seqs(seqs_by_subtype, maxlen)
...
# save arrays
train = h5py.File('flu_train.h5', 'w')
train.create_dataset('features', data = padded[0:37250])
train.create_dataset('labels', data = labels[0:37250])
train.close()

val = h5py.File('flu_val.h5', 'w')
val.create_dataset('features', data = padded[37250:49611])
val.create_dataset('labels', data = labels[37250:49611])
val.close()

test = h5py.File('flu_test.h5', 'w')
test.create_dataset('features', data = padded[49611:])
test.create_dataset('labels', data = labels[49611:])
test.close()
