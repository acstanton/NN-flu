...
# classification of full length fragments of influenza HA genes.                
...
# import data
def load_data():
    training_set = h5py.File('flu_train.h5', 'r')
    validation_set = h5py.File('flu_val.h5', 'r')
    test_set = h5py.File('flu_test.h5', 'r')
...
def cnn_model(x_ph, dropout_rate, kernel_size, num_kernels, ratio_fc_neurons):
    ...
    fc_units = round(1938 * ratio_fc_neurons)
    max_pool_stride = 2
    ...
    weights = {
        ...
        'wfc1' : tf.Variable(tf.truncated_normal(
            shape = [1 * 969 * out_channels, fc_units],
            mean = 0.0,
...
def training(x_train, y_train, x_val, y_val, hyperparam_config, num_epochs, batch_size,
             save_model=True, model_dir='models/fl_cnn/best_model'): 

    tf.reset_default_graph()
    
    # define input and output placeholders
    x_ph = tf.placeholder("float", [None, 1, 1938, 4], name="x_ph")
...
best_model_dir = "models/fl_cnn/best_model"
...
x_val_subset, y_val_subset = subsample_set(x_va, y_va, 500)

validation_loss = training(x_tr, y_tr, x_val_subset, y_val_subset, best_hyperparams, num_epochs, batch_size, save_model=True, model_dir=best_model_dir)
...
def plot_roc_curve(y, y_hat):
    ...
    plt.show()
    plt.savefig("fl_ROC.png", bbox_inches="tight")
...
def plot_pr_curve(y, y_hat):
    ...
    plt.savefig('fl_microavg_pr.png')
    ...
    plt.legend(lines, labels, bbox_to_anchor=(0,1.02,1,0.2), loc="lower left",
               prop=dict(size=10), mode="expand", borderaxespad=0, ncol=1)
    plt.savefig('fl_multiclass_pr.png', bbox_inches='tight')
...
def plot_confusion_matrix(y, y_hat):
    ...
    class_labels = ['H1', 'H2', 'H3', 'H4', 'H5', 'H6', 'H7', 'H8', 'H9', 'H10', 'H11', 'H12', 'H13', 'H14', 'H15', 'H16']
    ...
    fig.tight_layout()
    plt.title("Normalized confusion matrix for full length HA classification")
    np.set_printoptions(precision=2)
    plt.savefig("fl_confusion.png")
